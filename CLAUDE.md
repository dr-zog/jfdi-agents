...
- **`publish-to-github`** (fires on tag push matching `v<MAJOR>.<MINOR>.<PATCH>`): verifies the tag matches `plugin.json`'s version, `rsync`-stages the repo (excluding `PUBLISH_EXCLUDES`), clone-replaces the public GitHub repo's tracked contents, and commits a single "Release vX.Y.Z" commit under a separate identity. Idempotent.
