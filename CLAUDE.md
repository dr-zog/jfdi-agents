...
The full release pipeline lives in `.gitlab-ci.yml` at the repo root and runs in two stages:

- **`release-on-merge`** (fires on push to `main`): locates the merged MR via the GitLab API, reads its `semver::*` label, bumps `plugin.json`, commits the bump as the release bot with `--push-option=ci.skip`, tags `vX.Y.Z`, and pushes both. Refuses to double-bump (checks the last commit author against the bot's name) and refuses to overwrite an existing tag.
- **`publish-to-github`** (fires on tag push matching `v<MAJOR>.<MINOR>.<PATCH>`): verifies the tag matches `plugin.json`'s version, `rsync`-stages the repo (excluding `.git`, `.gitlab-ci.yml`, `vendor/`), clone-replaces the public GitHub repo's tracked contents, and commits a single "Release vX.Y.Z" commit under a separate identity. Idempotent.

Together the two stages mean a labelled MR merge is the only thing a human needs to do to ship a release to `github.com/dr-zog/jfdi-agents`.

### Release automation — what's still on the roadmap

- **`release-notes`**: generate a `CHANGELOG.md` entry on tag push from the MR titles since the previous tag. Worth adding once there are enough releases that scanning is painful.
- **Label-presence check on MR create**: a lightweight check that fails the pipeline (or adds an MR comment) if no `semver::*` label is set when the MR is opened, rather than only at merge time. Purely a UX improvement — the existing merge-time failure is already a hard gate.
